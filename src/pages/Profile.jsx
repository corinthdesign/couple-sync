...
export default function Profile() {
  ...
  useEffect(() => {
    async function loadProfile() {
      ...
      if (error) {
        ...
        setAvatarUrl(data.avatar_url ? supabase.storage.from('avatars').getPublicUrl(data.avatar_url).data.publicUrl : null);
        setEmail(user.email);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    const avatarPath = await handleAvatarUpload();
    ...
    <div className="p-4 max-w-md mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          />
          {avatarUrl && (
            <img src={avatarUrl} alt="Current Avatar" className="mt-2 h-24 w-24 rounded-full object-cover" />
          )}
        </div>
